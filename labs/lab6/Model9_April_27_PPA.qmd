---
title: "DRAFT Final Technical Appendix"
editor: visual
execute: 
  eval: false
---
...
test_data <- test_data %>% 
  mutate(predicted_class_03 = ifelse(predicted_prob > 0.03, 1, 0)) # if predicted probability>0.03, marked as 1 (delinquent)

cm_03 <- confusionMatrix(
  as.factor(test_data$predicted_class_03), # predicted delinquency
...
# Model 8: Confusion Matrix
test_data <- test_data %>% 
  mutate(predicted_class_15 = ifelse(predicted_prob > 0.15, 1, 0))

cm_15 <- confusionMatrix(
  as.factor(test_data$predicted_class_15),
...
ggplot() +
  ...
  labs(
    title = "Mid-High Income (Q3) Block Groups in Philadelphia",
    subtitle = "Highlighted block groups contain Q3 income properties",
    caption = "Source: OPA / Property Tax Data"
  ) +
  theme_void() +
  theme(
