...
### Model 7: using class weights on Delinquent (1)
...
```{r}
 test_data <- test_data %>% 
  mutate(
    predicted_prob = predict(logit_model7, newdata = test_data, type = "response")
  )
...
test_data <- test_data %>% 
  mutate(predicted_class_50 = ifelse(predicted_prob > 0.03, 1, 0)) # if predicted probability>0.03, marked as 1 (delinquent)
...
train_balanced_m8 <- bind_rows(
  train_data %>% filter(delinquent == 1),
  train_data %>% filter(delinquent == 0) %>% 
    slice_sample(n = n_delinquent * 4)         # mulitplied by 4!
)
...
test_data <- test_data %>%
  ...
  mutate(
    predicted_prob = predict(logit_model8, newdata = ., type = "response")
  )
...
# Model 8: Confusion Matrix
test_data <- test_data %>% 
  mutate(predicted_class_50 = ifelse(predicted_prob > 0.15, 1, 0))
...
roc_obj <- roc(
  ...
  predictor = test_data_clean$predicted_prob
)
...
## Model 7: Test Threshold Performance
...
threshold_results <- map_df(thresholds_to_test, function(thresh) {
  ...
  preds <- ifelse(test_data_clean$predicted_prob > thresh, 1, 0)
...
print(income_metrics_m8)

# Combine overall and by-group metrics
equity_analysis <- bind_rows(overall_metrics, income_metrics_m8)

# Display results
print("=" %>% rep(80) %>% paste0(collapse = ""))
print("EQUITY ANALYSIS: Model Performance by Income Group (Threshold = 0.15)")
print("=" %>% rep(80) %>% paste0(collapse = ""))
...
income_metrics_m8 %>%
  ...
  labs(
    title    = "Model8 Error Rates by Neighborhood Income Level (2020)",
...
```{r}
# Check what GEOID looks like in final_data_panelat block group level 
# Check what GEOID looks like in final_data_panel
# Check what GEOID looks like in final_data_panel
head(final_data_panel$GEOID)

# Step 1 - create geoid_lookup FIRST
...
test_data_m8 <- test_data_m8 %>%
  mutate(residual = delinquent - predicted_prob,
         opa_number = as.character(opa_number))

# Step 3 - join GEOID onto test_data_m8
test_data_m8 <- test_data_m8 %>%
  left_join(geoid_lookup, by = "opa_number")

# Step 4 - aggregate residuals to block group level
residuals_by_bg <- test_data_m8 %>%
  filter(!is.na(GEOID)) %>%
  group_by(GEOID) %>%
...
# Step 5 - join residuals to block group shapefile
...
# Step 1 - prepare block group residuals for spatial analysis
# drop NAs because spdep can't handle missing values
bg_residuals_clean <- block_group_residuals %>%
  filter(!is.na(mean_residual))

# Step 2 - define neighbors using contiguity (shared borders)
# using queen contiguity because we have polygons not points
bg_sp <- as(bg_residuals_clean, "Spatial")
neighbors <- poly2nb(bg_sp, queen = TRUE)
weights <- nb2listw(neighbors, style = "W", zero.policy = TRUE)
# style="W" = row standardized, each neighbor gets equal weight
# zero.policy=TRUE = allows block groups with no neighbors

# Step 3 - calculate spatial lag of residuals
# spatial lag = average residual of neighboring block groups
bg_residuals_clean$residual_lag <- lag.listw(weights, 
                                              bg_residuals_clean$mean_residual,
                                              zero.policy = TRUE)

# Step 4 - scatter plot of residual vs spatial lag of residual
# this is the Moran's I plot from the slides
ggplot(bg_residuals_clean, aes(x = residual_lag, y = mean_residual)) +
  ...
  theme_minimal()
# positive slope = spatial clustering in errors (bad)
# flat slope = random errors (good)

# Step 5 - run Moran's I test
moran_test <- moran.mc(
  bg_residuals_clean$mean_residual,  # your residuals
  weights,                            # spatial weights
  nsim = 999,                         # number of random permutations
  zero.policy = TRUE
)

# print results
moran_test$statistic   # Moran's I value (-1 to +1)
moran_test$p.value     # is it significant?

cat("Moran's I:", round(moran_test$statistic, 3), "\n")
cat("P-value:", round(moran_test$p.value, 3), "\n")
cat("Interpretation:", 
...
# Step 6 - visualize observed vs permuted Moran's I (from slides)
...
## Model 9: Final Trial - Household Spatial Effects

```{r}
# Model 9: Block Group Fixed Effects
# Diagnosed from Moran's I — residuals clustered at block group level
# Solution: replace neighborhood (Name) fixed effects with finer block group (GEOID) fixed effects

# Step 1 - rebuild model dataframe with GEOID included
final_model_df_m9 <- model_df %>%
  st_drop_geometry() %>% 
  select(
    opa_number,
    GEOID,
...
# Step 2 - train/test split for model 9
train_data_m9 <- final_model_df_m9 %>% filter(year < 2020)
test_data_m9  <- final_model_df_m9 %>% filter(year == 2020) %>% filter(lag_delinquent == 0)

cat("Training observations (2010-2019):", nrow(train_data_m9), "\n")
cat("Test observations (2020):", nrow(test_data_m9), "\n")

# Step 3 - balanced training data for model 9
n_delinquent_m9 <- sum(train_data_m9$delinquent == 1)

train_balanced_m9 <- bind_rows(
  train_data_m9 %>% filter(delinquent == 1),
  train_data_m9 %>% filter(delinquent == 0) %>% slice_sample(n = n_delinquent_m9 * 4)
)

prop_m9 <- prop.table(table(train_balanced_m9$delinquent))
w_m9 <- 1/prop_m9
train_balanced_m9$delinq_weight <- w_m9[as.character(train_balanced_m9$delinquent)]

table(train_balanced_m9$delinquent)

# Step 4 - run Model 9 with block group fixed effects
logit_model9 <- glm(
  delinquent ~ lag_delinquent +
    times_delinquent_before +
    log_marketvalue +
    ...
    in_emp +
    in_aff_buf +
    as.factor(GEOID),  # block group fixed effects — finer than neighborhood
  data = train_balanced_m9,
  family = "binomial",
  weights = delinq_weight
)

AIC9 <- AIC(logit_model9)  # AIC of the MODEL not the dataframe
summary(logit_model9)

```

```{r}
# Model 9: Predicted Probabilities
test_data_m9 <- test_data_m9 %>%
  filter(GEOID %in% unique(train_balanced_m9$GEOID)) %>%
  mutate(
    predicted_prob = predict(logit_model9, newdata = ., type = "response")
  )

summary(test_data_m9$predicted_prob)

ggplot(test_data_m9, aes(x = predicted_prob, fill = as.factor(delinquent))) +
  geom_histogram(bins = 50, alpha = 0.7, position = "identity") +
  scale_fill_manual(
    values = c("steelblue", "coral"), 
    labels = c("Not Delinquent", "Delinquent")
  ...
  labs(
    title = "Model 9: Predicted Probability of Delinquency in 2020", 
    subtitle = "Block Group Fixed Effects",
    x = "Predicted Probability",
    y = "Count",
    fill = "Actual Outcome"
  )
...
cm_m9 <- confusionMatrix(
  as.factor(test_data_m9$predicted_class_50),
  as.factor(test_data_m9$delinquent),
  positive = "1"
)
...
cat("\nKey Metrics at Threshold = 0.15:\n")
cat("Sensitivity:", round(cm_m9$byClass["Sensitivity"], 3),
    "— of all properties that became delinquent, how many did we catch?\n")
cat("Specificity:", round(cm_m9$byClass["Specificity"], 3),
    "— of all properties that stayed current, how many did we correctly spare?\n")
cat("Precision:", round(cm_m9$byClass["Precision"], 3),
    "— of properties we flagged, how many actually became delinquent?\n")
cat("Accuracy:", round(cm_m9$overall["Accuracy"], 3), "\n")
```

```{r}
# Model 9: Predicted Probabilities
test_data_m9 <- test_data_m9 %>%
  filter(Name %in% unique(train_balanced_m9$Name)) %>%
  mutate(
    predicted_prob = predict(logit_model9, newdata = ., type = "response")
  )
...
```{r}
roc_obj_m9 <- roc(
  response  = test_data_m9$delinquent,
  predictor = test_data_m9$predicted_prob
)

ggroc(roc_obj_m9, color = "steelblue", size = 1.5) +
  geom_abline(slope = 1, intercept = 1, linetype = "dashed", color = "gray50") +
  labs(
    title    = "ROC Curve: Model 9 Property Tax Delinquency",
    subtitle = paste0("AUC = ", round(auc(roc_obj_m9), 3)),
    x        = "1 - Specificity (False Positive Rate)",
    y        = "Sensitivity (True Positive Rate)"
  ) +
  coord_fixed()
...
```{r}
# Model 9: Threshold Testing
thresholds_to_test <- c(0.15, 0.3, 0.5, 0.7)

threshold_results_m9 <- map_df(thresholds_to_test, function(thresh) {
  preds <- ifelse(test_data_m9$predicted_prob > thresh, 1, 0)
...
print(threshold_results_m9)

threshold_results_m9 %>%
  select(threshold, sensitivity, specificity, precision) %>%
  pivot_longer(cols = c(sensitivity, specificity, precision),
               names_to = "metric",
               values_to = "value") %>%
  ...
  scale_x_continuous(breaks = thresholds_to_test) +
  labs(
    title    = "Model 9: Performance Metrics Across Thresholds",
    subtitle = "Block Group Fixed Effects + Undersampling + Class Weights",
    x        = "Probability Threshold",
    y        = "Metric Value",
    color    = "Metric"
  ) +
...
# Overall metrics
overall_metrics_m9 <- test_data_m9 %>%
  summarise(
    Group = "Overall",
    N = n(),
    Base_Rate = mean(delinquent),
...
income_metrics_m9 <- test_data_m9 %>%
  group_by(income_group) %>%
  summarise(
    N = n(),
    Base_Rate = mean(delinquent),
...
print(income_metrics_m9)

# Combine overall and by-group metrics
equity_analysis_m9 <- bind_rows(overall_metrics_m9, income_metrics_m9)

# Display results
print("=" %>% rep(80) %>% paste0(collapse = ""))
print("EQUITY ANALYSIS: Model 9 Performance by Income Group (Threshold = 0.15)")
print("=" %>% rep(80) %>% paste0(collapse = ""))
print(equity_analysis_m9, width = Inf)
print("=" %>% rep(80) %>% paste0(collapse = ""))
...
income_metrics_m9 %>%
  ...
  labs(
    title    = "Model 9 Error Rates by Neighborhood Income Level (2020)",
...
```{r}
# Model 9: Map of Lowest Income (Q1) Block Groups
# Q1 has the most balanced error rates — best model performance

# Step 1 - get Q1 opa numbers from test_data_m9
q1_opa_numbers_m9 <- test_data_m9 %>%
  filter(income_group == "Lowest Income (Q1)") %>%
  pull(opa_number) %>%
  unique()

# Step 2 - get GEOIDs for those properties from final_data_panel
q1_geoids_m9 <- final_data_panel %>%
  st_drop_geometry() %>%
  filter(opa_number %in% q1_opa_numbers_m9) %>%
  select(GEOID) %>%
  distinct() %>%
  mutate(GEOID = as.character(GEOID))

# Step 3 - flag block groups that have Q1 properties
q1_map_m9 <- block_group_shape %>%
  mutate(GEOID = as.character(GEOID)) %>%
  mutate(is_q1 = GEOID %in% q1_geoids_m9$GEOID)

# Step 4 - plot
ggplot() +
  geom_sf(data = PHL_LIMITS, fill = "grey90", color = NA) +
  geom_sf(data = q1_map_m9 %>% filter(!is_q1), fill = "grey80", color = NA, linewidth = 0.001) +
  geom_sf(data = q1_map_m9 %>% filter(is_q1), fill = "#1B2A4A", color = NA, linewidth = 0.001) +
  labs(
    title = "Model 9: Lowest Income (Q1) Block Groups in Philadelphia",
...
```{r}
# Model 9: Residuals Map
head(final_data_panel$GEOID)

# Step 1 - create geoid_lookup
geoid_lookup_m9 <- final_data_panel %>%
  st_drop_geometry() %>%
  select(opa_number, GEOID) %>%
  mutate(GEOID = as.character(GEOID),      # convert FIRST
         opa_number = as.character(opa_number)) %>%
  filter(!is.na(GEOID)) %>%               # THEN filter
  distinct()
...
# Step 3 - join GEOID onto test_data_m9
test_data_m9 <- test_data_m9 %>%
  left_join(geoid_lookup_m9, by = "opa_number")

# Step 4 - aggregate residuals to block group level
residuals_by_bg_m9 <- test_data_m9 %>%
...
# Step 5 - join residuals to block group shapefile
block_group_residuals_m9 <- block_group_shape %>%
  mutate(GEOID = as.character(GEOID)) %>%
  left_join(residuals_by_bg_m9, by = "GEOID")

# Step 6 - map it
ggplot() +
  geom_sf(data = PHL_LIMITS, fill = "grey90", color = NA) +
  geom_sf(data = block_group_residuals_m9,
          aes(fill = mean_residual),
          color = NA) +
  scale_fill_gradient2(
    low      = "lightgreen",
  ...
  labs(
    title    = "Model 9: Where Does Our Model Struggle?",
    subtitle = "Mean Prediction Residual by Block Group",
    caption  = "Red = model underestimates delinquency | Green = model overestimates delinquency"
  ) +
  theme_void() +
...
```{r}
# ── MORAN'S I DIAGNOSTIC FOR MODEL 9 ─────────────────────────────────────────
library(spdep)

# Step 1 - prepare block group residuals for spatial analysis
bg_residuals_clean_m9 <- block_group_residuals_m9 %>%
  filter(!is.na(mean_residual))

# Step 2 - define neighbors using contiguity (shared borders)
bg_sp_m9 <- as(bg_residuals_clean_m9, "Spatial")
neighbors_m9 <- poly2nb(bg_sp_m9, queen = TRUE)
weights_m9 <- nb2listw(neighbors_m9, style = "W", zero.policy = TRUE)

# Step 3 - calculate spatial lag of residuals
bg_residuals_clean_m9$residual_lag <- lag.listw(weights_m9,
                                                  bg_residuals_clean_m9$mean_residual,
                                                  zero.policy = TRUE)

# Step 4 - scatter plot of residual vs spatial lag
ggplot(bg_residuals_clean_m9, aes(x = residual_lag, y = mean_residual)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", color = "red") +
  labs(
    title    = "Model 9: Spatial Lag of Residuals",
    subtitle = "Is my block group's error correlated with neighboring block groups' errors?",
    x        = "Avg Residual of Neighboring Block Groups",
    y        = "Block Group Mean Residual"
  ) +
...
moran_test_m9$statistic
moran_test_m9$p.value

cat("Moran's I:", round(moran_test_m9$statistic, 3), "\n")
cat("P-value:", round(moran_test_m9$p.value, 3), "\n")
cat("Interpretation:",
    ifelse(moran_test_m9$p.value < 0.05,
           "Significant spatial clustering in errors — model missing spatial pattern",
           "Random errors — model adequately captures spatial relationships"), "\n")

# Step 6 - visualize observed vs permuted Moran's I
moran_df_m9 <- data.frame(sim_I = moran_test_m9$res[1:999])

ggplot(moran_df_m9, aes(x = sim_I)) +
  geom_histogram(binwidth = 0.01, fill = "gray70", color = "white") +
  geom_vline(aes(xintercept = moran_test_m9$statistic),
             color = "#FA7800", linewidth = 1.5) +
  labs(
    title    = "Model 9: Observed vs. Expected Moran's I",
    subtitle = "Orange line = observed | Gray = random permutations",
    x        = "Moran's I",
    y        = "Count"
  ) +
